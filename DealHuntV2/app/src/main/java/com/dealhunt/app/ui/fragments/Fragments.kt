...
private fun Fragment.goByGameId(id: String, title: String, thumb: String) {
    if (id.isBlank()) {
        Toast.makeText(context, "ID BOS (gameId)", Toast.LENGTH_LONG).show()
        return
    }
    // GECICI TEST: Activity acmak yerine sadece dialog goster
    try {
        android.app.AlertDialog.Builder(requireContext())
            .setTitle("Test Basarili")
            .setMessage("gameId=$id\ntitle=$title")
            .setPositiveButton("Tamam", null)
            .show()
    } catch (e: Throwable) {
        Toast.makeText(context, "DIALOG HATASI: ${e.message}", Toast.LENGTH_LONG).show()
    }
}

private fun Fragment.goByDealId(id: String, title: String, thumb: String) {
    if (id.isBlank()) {
        Toast.makeText(context, "ID BOS (dealId)", Toast.LENGTH_LONG).show()
        return
    }
    // GECICI TEST: Activity acmak yerine sadece dialog goster
    try {
